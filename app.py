...
# ----------------------------
    ...
    with st.form("form_hallazgo", clear_on_submit=True):
        ...
        with c1:
            ...
            modo_foto = st.radio("Foto", ["📷 Cámara", "🖼️ Galería"], horizontal=True, label_visibility="visible")
            if modo_foto == "📷 Cámara":
                foto = st.camera_input("Sacar foto", label_visibility="collapsed")
            else:
                foto = st.file_uploader("Elegir foto", type=["jpg", "jpeg", "png"], label_visibility="collapsed")
        with c2:
